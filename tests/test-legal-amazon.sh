...
echo "CREATING LEGAL AMAZON DATABASE TEST FOR SPECIFIC LOCAL OF INTEREST NAME AND YEAR..."
...
Query="CREATE TABLE private.legal_amazon_${loiname} AS SELECT * FROM private.legal_amazon_${loi} WHERE name LIKE '${loiname}';
	...
	CREATE TABLE private.intersection_${loiname} AS SELECT CASE WHEN ST_CoveredBy(mask.geom, l.geom) THEN ST_Multi(ST_MakeValid(mask.geom)) 
			...
			AS mask INNER JOIN private.legal_amazon_${loiname} l ON (ST_Intersects(mask.geom, l.geom));
...
psql postgresql://$user:$password@$host:$port/$database << EOF
	$Query
EOF > output_legal_amazon_${loiname}.txt