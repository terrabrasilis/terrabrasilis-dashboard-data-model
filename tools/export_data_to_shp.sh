...
BASE_YEARS=("2025" "2025" "2025" "2025" "2025" "2024" "2024" "2024")
...
do
    ...
    do
        if [[ "${CLS}" = "${YEAR_END}" ]]; then
            ...
            EXPORT_QUERY="SELECT fid, class_name, geom FROM ( "
            EXPORT_QUERY="${EXPORT_QUERY}    SELECT fid, class_name, geom FROM public.accumulated_deforestation_${YEAR_END}${table_suffix} "
            EXPORT_QUERY="${EXPORT_QUERY}    UNION "
            EXPORT_QUERY="${EXPORT_QUERY}    SELECT fid, class_name, geom FROM public.residual${table_suffix} "
            EXPORT_QUERY="${EXPORT_QUERY}    UNION "
            EXPORT_QUERY="${EXPORT_QUERY}    SELECT fid, class_name, geom FROM public.marco_eu_deforestation${table_suffix} WHERE def_prodes IS NULL"
            EXPORT_QUERY="${EXPORT_QUERY}) tb1 "
        else
            SHP_NAME="${TARGET}_${CLS}"
            EXPORT_QUERY="SELECT fid, class_name, geom FROM $schema.yearly_deforestation${table_suffix} WHERE class_name='d${CLS}'"
        fi;
